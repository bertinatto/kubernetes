...
func (t *volumeLimitsTestSuite) defineTests(driver TestDriver, pattern testpatterns.TestPattern) {
	...
	ginkgo.It("should support volume limits [Slow][Serial]", func() {
		...
		if dDriver = driver.(DynamicPVTestDriver); dDriver == nil {
			ginkgo.Fail("Test driver does not provide dynamically created volumes")
		}
		...
		pod := framework.MakeSecPod(l.ns.Name, l.pvcs, nil, false, "", false, false, framework.SELinuxLabel, nil)
		// Use affinity to schedule everything on the right node
		selection := framework.NodeSelection{}
		framework.SetAffinity(&selection, nodeName)
		...
		pod = framework.MakeSecPod(l.ns.Name, []*v1.PersistentVolumeClaim{l.resource.pvc}, nil, false, "", false, false, framework.SELinuxLabel, nil)
		// Use affinity to schedule everything on the right node
		framework.SetAffinity(&selection, nodeName)
		pod.Spec.Affinity = selection.Affinity
